...
namespace AiMeetingSummariser.Api.Application.Features.Meetings.SearchMeetings;

public record SearchMeetingsQuery(string SearchTerm, int Page = 1, int PageSize = 10) : IRequest<ResponseWrapper<PaginatedList<MeetingDto>>>;

public class SearchMeetingsHandler : IRequestHandler<SearchMeetingsQuery, ResponseWrapper<PaginatedList<MeetingDto>>>
{
    private readonly AppDbContext _context;
    private readonly IHttpContextAccessor _httpContextAccessor;
    
    public SearchMeetingsHandler(AppDbContext context, IHttpContextAccessor httpContextAccessor)
    {
        _context = context;
        _httpContextAccessor = httpContextAccessor;
    }
    
    public async Task<ResponseWrapper<PaginatedList<MeetingDto>>> Handle(SearchMeetingsQuery request, CancellationToken cancellationToken)
    {
        ...
        return ResponseWrapper<PaginatedList<MeetingDto>>.SuccessResponse(paginatedList, "Meetings searched successfully");
    }
}